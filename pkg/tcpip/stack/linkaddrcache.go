...
func (c *linkAddrCache) add(k tcpip.FullAddress, v tcpip.LinkAddress) {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	entry := c.getEntryLocked(k)
	entry.linkAddr = v

	entry.changeState(ready, expiration)
}

func (c *linkAddrCache) getEntryLocked(k tcpip.FullAddress) *linkAddrEntry {
	entry, ok := c.mu.cache[k]
	if !ok {
		if len(c.mu.cache) == linkAddrCacheSize {
			entry = c.mu.list.Back()

			delete(c.mu.cache, entry.addr)
	...
	if ok {
		c.mu.list.Remove(entry)
	}
	c.mu.list.PushFront(entry)
	return entry
}
...
func newLinkAddrCache(ageLimit, resolutionTimeout time.Duration, resolutionAttempts int) *linkAddrCache {
	...
	c.mu.cache = make(map[tcpip.FullAddress]*linkAddrEntry, linkAddrCacheSize)
	return c
}
