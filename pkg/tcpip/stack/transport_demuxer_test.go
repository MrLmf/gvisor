...
func newDualTestContextMultiNic(t *testing.T, mtu uint32, linkEpNames []string) *testContext {
	...
	for i, linkEpName := range linkEpNames {
		...
		if err := s.CreateNamedNIC(nicid, linkEpName, id); err != nil {
			t.Fatalf("CreateNIC failed: %v", err)
		}
		linkEPs[linkEpName] = linkEP
...
func TestDistribution(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			map[string][]float64{
				// Injected packets on dev0 get distributed among endpoints bound to dev0.
				"dev0": []float64{0.5, 0.5, 0, 0, 0, 0},
				// Injected packets on dev1 get distributed among endpoints bound to dev1 or unbound.
