...
func networkToHost32FromBytes(h []byte) uint32 {
	return usermem.ByteOrder.Uint32(h)
}

func writeInetAddr(w io.Writer, a linux.SockAddrInet) {
	// linux.SockAddrInet.Port is stored in the network byte order and is
	// printed in host byte order by linux in get_tcp4_sock() and
	// udp4_format_sock().
	port := networkToHost16(a.Port)

	// linux.SockAddrInet.Addr is stored as a byte slice in big-endian order
	// (i.e. most-significant byte in index 0) and is again printed in host byte
	// order.
	addr := networkToHost32FromBytes(a.Addr[:])
